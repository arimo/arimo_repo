...
class GenericDBO {
	...
	private function _initDB(){
		...
		if(TRUE === @property_exists($this, "DSN") && isset($this->DSN) && NULL !== $this->DSN && strlen($this->DSN) > 0){
			...
			// 継承元クラス名に応じたDSN定義を取得
			if(0 < strlen($calledClassName) && __CLASS__  != $calledClassName && class_exists("Config") && NULL !== Config::constant(strtoupper($calledClassName) . "_DSN")){
				$dsn = Config::constant(strtoupper($calledClassName) . "_DSN");
			}
			elseif(class_exists("Config") && NULL !== Config::constant("DB_DSN")){
				$dsn = Config::DB_DSN;
			}
			else{
				$dsn = DB_DSN;
		...
		if(!(isset(self::$_DBInstance[$calledClassName]) && is_object(self::$_DBInstance[$calledClassName]))) {
			...
			// それを再利用してしまうケースがあるので、まず再利用コネクションをロールバッする事に注意！！！
