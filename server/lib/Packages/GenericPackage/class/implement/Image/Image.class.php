 ...
 *
 * <code>
 * $result = ImageFIleInfo::getInfoFromFile('test.jpg');
 * if($result->isError())
 * {
 ...
 * 原因不明のエラー
 */
define('IMAGEFILEINFO_ERROR_UNKNOWN',     1);

/**
 * ヘッダー情報の取得に失敗
 */
define('IMAGEFILEINFO_ERROR_HEADERDATA',  2);


/**
 * サポートされていないフォーマット
 */
define('IMAGEFILEINFO_ERROR_UNSUPPORTED', 3);
 ...
 * @access private
 */
define('IMAGEFILEINFO_JPEG_TYPE_BASELINE',    0);

/**
 * プログレッシブ形式のJPEG
 * @access private
 */
define('IMAGEFILEINFO_JPEG_TYPE_PROGRESSIVE', 2);
 ...
 *
 * $result = ImageFileInfo::getInfoFromFile('/xxx/xxx.xxx');
 */
class ImageInfo
{
	/**
	...
	function &getInfoFromFile($filename)
	{
		...
		$res = &ImageFileInfo::getInfoFromData($buffer);
		return $res;
	}
	...
	function &getInfoFromData(&$data)
	{
		...
		if(ImageFileInfo_Jpeg::isValid($data))
		{
			//JPEG画像の場合
			$obj = new ImageFileInfo_Jpeg();
		}
		elseif(ImageFileInfo_Png::isValid($data))
		{
			//PNG画像の場合
			$obj = new ImageFileInfo_Png();
		}
		elseif(ImageFileInfo_Gif::isValid($data))
		{
			//GIF画像の場合
			$obj = new ImageFileInfo_Gif();
		}
		else
		{
			//サポートしていないか、無効な画像データ
			$image_info['error'] = IMAGEFILEINFO_ERROR_UNSUPPORTED;
...
class ImageInfoResult
{
	...
	function ImageInfoResult($image_info)
	{
		...
		if(!is_array($image_info))
		{
			...
			$this->error_code = IMAGEFILEINFO_ERROR_UNKNOWN;
			return;
		}
	...
	/**
	 * エラーコードを返す
	 * @return int エラーコード。IMAGEFILEINFO_ERROR_XXX
	 */
...
class ImageUtil
{
	...
	function crop($argImageResouceID, $argImageInfoObj, $argCropLeft = 0, $argCropTop = 0, $argCropRight = 0, $argCropBottom = 0){

		$info = $argImageInfoObj;
		$image = $argImageResouceID;
	...
	function resize( $argImageResouceID, $argImageInfoObj, $width = 0, $height = 0, $proportional = false)
	{
		if ( $height <= 0 && $width <= 0 ) {
			return false;
		}
...
class ImageFileInfo_Jpeg
{
	...
	function getInfo(&$data, &$image_info)
	{
		...
		if($sof0_pos)
		{
			...
			$this->jpeg_type = IMAGEFILEINFO_JPEG_TYPE_BASELINE;
			$sof_pos = $sof0_pos;
		}
		elseif($sof2_pos)
		{
			//SOF0が見つかった場合はプログレッシブJPEG
			$this->jpeg_type = IMAGEFILEINFO_JPEG_TYPE_PROGRESSIVE;
			$sof_pos = $sof2_pos;
		}
		else
		{
			//どちらもみつからない場合はエラー
			$image_info['error'] = IMAGEFILEINFO_ERROR_HEADERDATA;
			return false;
		}
